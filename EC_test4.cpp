...
VectorXcd greens(double k_x, double k_z, double a, dcomp omega, int N, dmat &u,
	...
	GLu = gs(OMu, Tu_B);
	GLd = gs(OMd, Td_B);
	miniOMu = omega*small_I-uu_11;
	miniOMd = omega*small_I-ud_11;
	...
	GRu_C = gr(miniOMu, Tu_21, tu_15);
	GRd_C = gr(miniOMd, Td_21, td_15);
	...
	Matrix<dcomp, 27, 27> Ju, Jd, Jlu, Jld, H_B, H_C;
	Matrix<dcomp, 27, 27> bigeye = Matrix<dcomp, 27, 27>::Identity();

	Ju.topLeftCorner(18, 18) = GLu;
	Ju.topRightCorner(18, 9) = Matrix<dcomp, 18, 9>::Zero();
	Ju.bottomLeftCorner(9, 18) = Matrix<dcomp, 9, 18>::Zero();
	Ju.bottomRightCorner(9, 9) = (1./omega)*small_I;

	Jd.topLeftCorner(18, 18) = GLd;
	Jd.topRightCorner(18, 9) = Matrix<dcomp, 18, 9>::Zero();
	Jd.bottomLeftCorner(9, 18) = Matrix<dcomp, 9, 18>::Zero();
	Jd.bottomRightCorner(9, 9) = (1./omega)*small_I;

	H_B.topLeftCorner(18, 18) = Matrix<dcomp, 18, 18>::Zero();
	H_B.topRightCorner(9, 9) = t_15;
	H_B.block<9, 9>(18, 9) = T_21.adjoint();
	H_B.bottomLeftCorner(9, 9) = t_15.adjoint();
	H_B.block<9, 9>(9, 18) = T_21;
	H_B.bottomRightCorner(9, 9) = u_11;

	H_C = H_B;
	H_C.block<9, 9>(18, 9) = u_12.adjoint();
	H_C.block<9, 9>(9, 18) = u_12;
...
//adlayer layer 2 from layer 1 to spacer thickness, N
	for (int it=1; it < N; ++it){
		if (it%2 == 0){
			Jlu = ((bigeye - Ju*H_B).inverse())*Ju;
			Jld = ((bigeye - Jd*H_B).inverse())*Jd;
			GLu = Jlu.bottomRightCorner(18, 18);
			GLd = Jld.bottomRightCorner(18, 18);
			Ju.topLeftCorner(18, 18) = GLu;
			Jd.topLeftCorner(18, 18) = GLd;
			Rsigma_0_u = (I-GRu_B*T_Bdagg*GLu*T_B);
			Rsigma_0_d = (I-GRd_B*T_Bdagg*GLd*T_B);
		...
		if (it%2 == 1){
			Jlu = ((bigeye - Ju*H_C).inverse())*Ju;
			Jld = ((bigeye - Jd*H_C).inverse())*Jd;
			GLu = Jlu.bottomRightCorner(18, 18);
			GLd = Jld.bottomRightCorner(18, 18);
			Ju.topLeftCorner(18, 18) = GLu;
			Jd.topLeftCorner(18, 18) = GLd;
			Rsigma_0_u = (I-GRu_C*T_Bdagg*GLu*T_B);
			Rsigma_0_d = (I-GRd_C*T_Bdagg*GLd*T_B);
			Rsigma_PI_u = (I-GRd_C*T_Bdagg*GLu*T_B);
			Rsigma_PI_d = (I-GRu_C*T_Bdagg*GLd*T_B);
		}
