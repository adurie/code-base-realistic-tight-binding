...
c                                    npmax - max No of points per plane
      ...
      complex*16 ec
      common/shift/xmgo_shift,au_shift,ag_shift
      common /fermi/ ef
     ...
     $  dnn(nnx,nlayx,nlayx),ddnn(numatx,numatx,nnx),itype(nsubx,nlayx)
      ...
      dimension xn(3)
      common/atmap/itypefe(3),itypeau(3)
...
c     DATA
      ...
      nins=15
      mlay=1     !! No of substrate layers on each side of SGF
      numat=2    !! No of atom types: one for each element..used in param

      nlay=nins+6  !!! total No of layers inc 4 lead layers
      ndiff=nins

      ef=-0.02490135d0    !! for Fe
CCC   au_shift=ef-0.5380d0
...
c       Out of plane lattice vector
        aa3(1,ilay)=0.d0
        aa3(2,ilay)=0.d0
        aa3(3,ilay)=(ilay-1.d0)*1/sqrt(2.d0)
...
c       Sublattice
        ...
        vsubat(3,2,ilay)=-1.d0/(2*sqrt(2.d0))

c       Atom types
        do kk=1,nsubat
          itypeat(kk,ilay)=1    !! Fe
        enddo
      enddo
c
...
c       Sublattice
        ...
        vsub(1,2,ilay)=0.5d0
        vsub(2,2,ilay)=0.d0
        vsub(3,2,ilay)=-1.d0/(2*sqrt(2.d0))
...
c       Atom types
        itype(1,ilay)=1   !!!   Fe
        itype(2,ilay)=1   !!!   Fe IF with Au
      enddo

      do ilay=nins+5,nins+6
c       Out of plane lattice vector
        a3(1,ilay)=a3(1,ilay-1)+0.d0
        a3(2,ilay)=a3(2,ilay-1)+0.d0
        a3(3,ilay)=a3(3,ilay-1)+1.d0/(sqrt(2.d0))
...
C     Fe-Fe
      ddnn(1,1,1)=0.612372435695794d0
      ddnn(1,1,2)=0.707106781186548d0
      ddnn(1,1,3)=1.00000000000000d0

C     Au-Au
      ddnn(2,2,1)=0.707106781186548d0
      ddnn(2,2,2)=1.d0
      ddnn(2,2,3)=1.22474487139159d0

C     Fe-Au
      ddnn(1,2,1)=0.612372435695794d0
      ddnn(1,2,2)=0.707106781186548d0
      ddnn(1,2,3)=1.d0
      ddnn(2,1,1)=0.612372435695794d0
      ddnn(2,1,2)=0.707106781186548d0
      ddnn(2,1,3)=1.d0

C     =================================================================
C     !!!!!!! OUTPUT ATOMIC POSITIONS FOR V_SIM VIEWER !!!!!!
...
c
      ...
      write(*,*)' (UP + DOWN - 2*AF)'
      do in=0,ndiff
        write(*,*)2*in-1,-(vcuu(in)+vcdd(in)-vcdu(in)-vcud(in))/nsub
      enddo
      endif
c
...
c
      s0(1,+1) =  0.514d0
      p0(1,+1) =  1.118d0
      d0t(1,+1) = -0.089d0 - dels2
      d0e(1,+1) = -0.068d0 - dels2
c
c     first n.n.
c
      sss(1,1,+1) = -0.10605d0
...
c
        ...
        d0t(2,isp) =0.12249d0 + ag_shift
        d0e(2,isp) =0.12006d0 + ag_shift
c
c       first n.n.
c
