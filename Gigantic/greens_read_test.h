...
/* typedef map<Vector3d, MatrixXcd, less<Vector3d>, aligned_allocator<pair<const Vector3d, MatrixXcd>>> maaap; */
typedef map<vector<double>, MatrixXcd, less<vector<double>>, aligned_allocator<pair<const vector<double>, MatrixXcd>>> maaap;

/* std::map< */
/*   Eigen::Vector3d, */
/*   MatrixXcd, */
/*   std::function<MatrixXcd(const Eigen::Vector3d&,const Eigen::Vector3d&)> > */
/*   m([](const Vector3d & a, const Vector3d & b)->MatrixXcd */
/*   { */
...
/* { */
/*      template<> */
/*      std::less<Eigen::Vector3d>(Eigen::Vector3d const& a, Eigen::Vector3d const& b) */
/*      { */
...
MatrixXcd extract(const Vector3d &d, const Vector3d &xk, const Vector3d &dpar, int nspin, const maaap &H){
      MatrixXcd rt(nspin,nspin);
      rt.fill(0.);
      double dk;
      dcomp zexdk, i;
      ...
      zexdk=cos(dk) + i*sin(dk);
      vector<double> dvec;
      dvec.emplace_back(d(0));
      dvec.emplace_back(d(1));
      dvec.emplace_back(d(2));
      rt = H.find(dvec)->second;
      return rt*zexdk;
}

/* MatrixXcd read(int nn, Vector3d &dvec, double dd, const Vector3d &xk, Vector3d &dpar, int nspin, int ispin){ */
/*       string input; */
/*       if (ispin == +1) */
...
MatrixXcd helement(int n1, int n2, int isub, int jsub, const Vector3d &xk, int ispin, int ndim,
	       	...
	       	const vVXd &itypeat, const vMXd &ddnn, const maaap &iron_up,
		const maaap &iron_dn, Args&&... params){
...
//         First construct d_{//}
	    ...
	    if ((ind1 == 0) && (ind2 == 0)){
              if (ispin == +1)
	        zh = zh + extract(d, xk, dpar, nspin, iron_up);
              if (ispin == -1)
	        zh = zh + extract(d, xk, dpar, nspin, iron_dn);
	    }
	    else
