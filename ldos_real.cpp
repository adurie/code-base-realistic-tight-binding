...
double temp(double k_y, void * alpha) {
	...
	Ec = E + im;
	double result;
	result = kspace(&greens, 3, 1e-4, 2.*params.a, k_y, Ec, params.u, params.t_1, params.t_2, params.t_3,
...
int main(){
	...
	F.function = &temp;
	F.params = &params;
	const double start = -M_PI/(2.*params.a);
	const double end = M_PI/(2.*params.a);
	double k_y = 0;
	for (params.j = -0.2; params.j<1.3; params.j=params.j+0.01){
		gsl_integration_qags(&F, start, end, 0, 1e-4, 1000, w, &result, &error);
		cout<<error<<endl;
