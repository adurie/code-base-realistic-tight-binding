...
//mobius transformation layer 2 from layer 1 to spacer thickness, N
	...
	for (int it=0; it < N; ++it){
		tmpu = (A1.array().pow((it+1)/4.).matrix()).asDiagonal()*fu*(A2.array().pow(-(it+1)/4.).matrix()).asDiagonal();
		tmpd = (A1.array().pow((it+1)/4.).matrix()).asDiagonal()*fd*(A2.array().pow(-(it+1)/4.).matrix()).asDiagonal();
...
int main(){
	...
	for (int ii=0; ii < N ; ++ii)
		Myfile << (ii+1)/4. <<" ,  "<< -2.*M_PI*result[ii] << endl;
